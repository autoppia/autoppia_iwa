...
LLM_TIMEOUT_SECONDS = _env_float("AUTOPPIA_DYNAMIC_LLM_TIMEOUT", 20.0)
LLM_RETRY_DELAY_SECONDS = _env_float("AUTOPPIA_DYNAMIC_LLM_RETRY_DELAY", 1.5)
LLM_RESPONSE_PREVIEW_CHARS = _env_int("AUTOPPIA_DYNAMIC_LLM_PREVIEW_CHARS", 160)
...
class DynamicPageResult:
    ...
    runtime_maps: dict[int | None, dict[str, Any] | None] = field(default_factory=dict)
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "runtime_maps": self.runtime_maps,
        }
...
async def _evaluate_single_page(
    *,
    ...
    map_runs: dict[int | None, list[dict[str, Any] | None]] = {}

    candidate_patterns = list(_candidate_patterns(page_spec))
    for pattern in candidate_patterns:
        html, url, error, runtime_map = await _capture_html(page, base_url, pattern, seed=None, config=config)
        if error:
        ...
        map_runs.setdefault(None, []).append(runtime_map)
        break
    ...
    await _collect_runs(page, base_url, working_pattern, None, config.seedless_repeats - 1, config, html_runs, map_runs)

    # Seeded runs
    for seed in config.seeds:
        await _collect_runs(page, base_url, working_pattern, seed, config.repeats_per_seed, config, html_runs, map_runs)
    ...
    if llm_service and html_runs.get(None) and any(html_runs.get(seed) for seed in config.seeds):
        llm_pass, llm_feedback = await _llm_assess_variation_with_retry(
            llm_service=llm_service,
...
async def _collect_runs(
    ...
    repeats: int,
    config: DynamicGateConfig,
    html_storage: dict[int | None, list[str]],
    map_storage: dict[int | None, list[dict[str, Any] | None]],
) -> None:
    if repeats <= 0:
        return
    for _ in range(repeats):
        html, _, error, runtime_map = await _capture_html(page, base_url, pattern, seed=seed, config=config)
        if error:
            html_storage.setdefault(seed, [])
            html_storage[seed].append("")
            map_storage.setdefault(seed, [])
            map_storage[seed].append(None)
            logger.warning("Dynamic capture failed for seed=%s pattern=%s: %s", seed, pattern, error)
            continue
        html_storage.setdefault(seed, [])
        html_storage[seed].append(html or "")
        map_storage.setdefault(seed, [])
        map_storage[seed].append(runtime_map)
...
async def _llm_assess_variation(llm_service, page_id: str, base_html: str, seed_htmls: list[tuple[int, str]], heuristics: dict) -> tuple[bool | None, str | None]:
    ...
    try:
        raw_response = await asyncio.wait_for(
            llm_service.async_predict(
                messages=messages,
                json_format=True,
                schema=LLM_DYNAMIC_SCHEMA,
                return_raw=False,
            ),
        ...
        if isinstance(raw_response, str):
            ...
            preview = cleaned[:LLM_RESPONSE_PREVIEW_CHARS]
        elif isinstance(raw_response, dict):
            preview = str(raw_response)[:LLM_RESPONSE_PREVIEW_CHARS]
        if preview:
            logger.warning(f"LLM dynamic assessment failed: {exc} | preview={preview!r}")
        else:
            logger.warning(f"LLM dynamic assessment failed: {exc}")
        return None, f"LLM error: {exc}"


async def _llm_assess_variation_with_retry(
    llm_service,
    page_id: str,
    base_html: str,
    seed_htmls: list[tuple[int, str]],
    heuristics: dict,
    attempts: int = 3,
) -> tuple[bool | None, str | None]:
    last_result: tuple[bool | None, str | None] = (None, None)
    for attempt in range(1, attempts + 1):
        result = await _llm_assess_variation(llm_service, page_id, base_html, seed_htmls, heuristics)
        last_result = result
        if result[0] is not None:
            return result
        if attempt < attempts:
            logger.warning(f"LLM dynamic assessment returned no verdict (attempt {attempt}/{attempts}). Retrying…")
            backoff = LLM_RETRY_DELAY_SECONDS * attempt
            if backoff > 0:
