#!/usr/bin/env python3
"""Generate evaluation episodes using the BaseAgentWithCache via Simple API."""
...
# Ensure action classes are registered
from autoppia_iwa.src.execution.actions import actions as _actions  # noqa: F401

DEFAULT_CACHE_DIR = "data/tasks_cache"
...
def _start_agent(port: int, agent_number: int) -> subprocess.Popen | None:
    ...
    logger.info(f"Starting cache agent (agent_number={agent_number}) on port {port}")
    ...
    try:
        ...
        for task in tasks:
            ...
            logger.info(
                ...
                eid=episode_payload["episode_id"],
                score=episode_payload["final_score"],
                raw=episode_payload["raw_score"],
                actions=len(episode_payload["steps"]),
            )
    finally:
        if proc is not None:
