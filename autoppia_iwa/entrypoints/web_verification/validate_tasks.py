...
async def is_task_consistent(task, llm_service):
    ...
    for constraint in constraints:
        ...
        if value.lower() not in prompt.lower() and len(value) > 2:
...
async def run_validation(project_id, llm_service):
    ...
    for uc_name, uc_content in use_cases.items():
        ...
        for task in tasks:
            ...
            res_item = {"task_id": task.get("task_id"), "use_case": uc_name, "is_consistent": valid, "reason": msg, "prompt": task.get("prompt")}
