...
For most cases you will just need 1 test in the list especially if there is just 1 thing that has to be validated.
Do not add overlapping tests that validate the same thing or partial things that other tests already validate in a complete way.
...
- **equals**: Field must exactly match the value (default when operator is omitted)
...
- **not_in_list**: Value must not be any of the listed options (specify value as an **array**)

---

## SPECIAL CLARIFICATION FOR LIST-BASED CONSTRAINTS
When a task prompt includes a request like:

> "Give me information about a book published in a year that is NOT in the list [2023, 2022, 2011]"

Then the `not_in_list` test should only validate that those **exact** years are excluded — and **no additional exclusions** should be inferred.

❌ INCORRECT:
"year": {{
  "value": [2010, 2013, 2014, 2015, 2016, 2017, 2018, 2019, 2020, 2021],
  "operator": "not_in_list"
}}

✅ CORRECT:
"year": {{
  "value": [2023, 2022, 2011],
  "operator": "not_in_list"
}}
