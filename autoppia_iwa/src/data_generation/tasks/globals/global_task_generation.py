...
class GlobalTaskGenerationPipeline:
    ...
    async def generate(self, num_use_cases: int, prompts_per_use_case: int | None = 5, use_cases: list[str] | None = None) -> list[Task]:
        """
        ...
        auto_prompt_budget = prompts_per_use_case is None or prompts_per_use_case <= 0
        budget_label = prompts_per_use_case if not auto_prompt_budget else "auto"
        _log_task_generation(f"Generating tasks for all use cases with {budget_label} tasks each. Selected {len(web_use_cases)} use cases.")

        for use_case in web_use_cases:
            _log_task_generation(f"Generating tasks for use case: {use_case.name}", context="USE_CASE")
            try:
                prompts_for_case = prompts_per_use_case
                if auto_prompt_budget:
                    example_count = len(getattr(use_case, "examples", []) or [])
                    prompts_for_case = example_count or 1
                tasks_for_use_case = await self.generate_tasks_for_use_case(use_case, prompts_for_case)
