...
async def _get_restaurants_dataset(
    task_url: str | None,
    dataset: list[dict[str, Any]] | dict[str, list[dict[str, Any]]] | None,
) -> list[dict[str, Any]]:
    """Return restaurants list: fetch by seed if dataset is None, else use dataset (list or dict with 'restaurants' key)."""
    if dataset is None or dataset == {}:
        seed = get_seed_from_url(task_url)
        return await fetch_data(seed_value=seed)
    if isinstance(dataset, list) and dataset:
        return dataset
    if isinstance(dataset, dict) and dataset.get("restaurants"):
...
async def generate_view_restaurant_constraints(
    ...
    if test_types == "data_extraction_only":
        ...
        result = _build_data_extraction_result(restaurant, VISIBLE_FIELDS_RESTAURANT_DETAIL, question_fields_override=["name"])
...
async def generate_search_restaurant_constraints(
    ...
    if test_types == "data_extraction_only":
        ...
        if not restaurants:
            return []
        restaurant = random.choice(restaurants)
        # item_with_query = {**restaurant, "name": restaurant.get("name") or ""}
        result = _build_data_extraction_result(restaurant, VISIBLE_FIELDS_SEARCH_RESTAURANT, question_fields_override=["name"])
        return result if result is not None else []
...
def generate_restaurant_constraints(
    *,
    ...
    dataset = dataset.get("restaurants", []) if isinstance(dataset, dict) else list(dataset) if dataset else []
    if not dataset:
        raise ValueError("dataset cannot be empty")
