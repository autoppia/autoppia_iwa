...
DATA_EXTRACTION_USE_CASES: list[DataExtractionUseCaseDefinition] = [
    DataExtractionUseCaseDefinition(name="FIND_RIDE", description="Find a ride option from route details."),
    DataExtractionUseCaseDefinition(name="FIND_DATE", description="Find date information for a ride option."),
]

CASE_CONFIGS: dict[str, DataExtractionCaseConfig] = {
    "FIND_RIDE": DataExtractionCaseConfig(
        answer_keys=("ride_name", "name", "title"),
        identifier_keys=("location", "destination", "eta", "price", "scheduled"),
        answer_label="ride",
        entity_label="ride option",
    ),
    "FIND_DATE": DataExtractionCaseConfig(
        answer_keys=("eta", "waitTime", "time"),
        identifier_keys=("name", "price", "seats", "desc"),
...
async def _load_rows_for_use_case(*, seed: int, use_case_name: str) -> list[dict]:
    return await fetch_data(entity_type="rides", method="shuffle", seed_value=int(seed), count=80)
