...
DATA_EXTRACTION_USE_CASES: list[DataExtractionUseCaseDefinition] = [
    DataExtractionUseCaseDefinition(name="FIND_EVENT", description="Find an event from calendar data."),
]

CASE_CONFIGS: dict[str, DataExtractionCaseConfig] = {
    "FIND_EVENT": DataExtractionCaseConfig(
        answer_keys=("label", "title", "name"),
        identifier_keys=("date", "start", "location", "calendar", "description"),
        answer_label="event",
