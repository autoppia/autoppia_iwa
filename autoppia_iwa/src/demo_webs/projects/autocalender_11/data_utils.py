...
async def fetch_events_data(seed_value: int | None = None, count: int = 200) -> list[dict]:
    ...
    from .main import FRONTEND_PORT_INDEX, autocalender_project

    field_mapping = {"allDay": "all_day", "recurrenceEndDate": "recurrence_end_date"}

    project_key = f"web_{FRONTEND_PORT_INDEX + 1}_{autocalender_project.id}"

    items = await load_dataset_data(
        backend_url=autocalender_project.backend_url,
