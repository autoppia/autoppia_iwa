...
def _log_action_execution(message: str, web_agent_id: str | None = None):
    log_event("EVALUATION", message, context="ACTION EXECUTION", web_agent_id=web_agent_id)


def _log_gif_creation(message: str, web_agent_id: str | None = None):
    log_event("EVALUATION", message, context="GIF CREATION", web_agent_id=web_agent_id)
...
def _log_evaluation_fallback(message: str, context: str = "GENERAL"):
    """Backward-compatible alias for the simplified evaluation logger."""
    _log_evaluation_event(message, context=context)
