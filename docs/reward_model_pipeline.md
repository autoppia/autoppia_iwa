...
data/inputs/reward_model/   # Reward model training data and checkpoints
```
...
Otherwise, run a tiny benchmark batch with JS instrumentation enabled to populate `data/inputs/reward_model/raw_traces`, or replay real leaderboard solutions via `replay_leaderboard_solutions.py` (recommended for production-like data):
...
1. Export evaluation episodes (`EvaluationEpisode` jsonl) into `data/inputs/reward_model/raw_evaluations/`.
   Each line must include the action results, browser snapshots, and final test
   score for the episode.
2. Provide split definitions in `data/inputs/reward_model/splits/`:
...
1. **Extract observations & LLM semantics**
   ...
   - Cleans DOM text, tokenises URLs, and stores observation JSON + cached LLM
     semantic labels in `data/inputs/reward_model/features/`.
...
2. **Build preference pairs**
   ...
   - Uses final scores to select positive/negative steps and stores pairs in
     `data/inputs/reward_model/pairs/pairs.jsonl`.
...
3. **Train semantic encoder (optional but recommended)**
   ...
   - Distils LLM semantic labels into a lightweight encoder. The checkpoint is
     saved under `data/inputs/reward_model/ckpts/semantic_encoder.pt`.

4. **Build DOM + JS event features (new)**
   ```bash
   python -m autoppia_iwa.src.rl.score_model.cli.build_dom_event_features \
       --traces-dir data/inputs/reward_model/raw_traces \
       --output-dir data/inputs/reward_model/features \
       --split-dir data/inputs/reward_model/splits
   ```
...
4. **Train reward model**
   ...
   - Optimises the preference win-rate and success BCE. Produces the reward model
     checkpoint at `data/inputs/reward_model/ckpts/reward_model.pt`.

5. **(Optional) Train DOM-event baseline**
   ```bash
   python -m autoppia_iwa.src.rl.score_model.cli.train_dom_event_baseline \
       --steps-file data/inputs/reward_model/features/train_dom_event_steps.jsonl
   ```
   - Fits a quick Gradient Boosting classifier on the per-step dataset to sanity-check feature quality; checkpoint saved to `data/inputs/reward_model/ckpts/dom_event_baseline.pkl`.
...
5. **Integrate with PPO**
   ...
   - Blend shaped reward with binary test reward in the training loop:
     ```python
     blender = RewardBlender("data/inputs/reward_model/ckpts/reward_model.pt", alpha=0.5, beta=0.5)
...
- Cached artefacts (`data/inputs/reward_model/features`, `data/inputs/reward_model/llm_labels`, `data/inputs/reward_model/pairs`) should be
