...
@pytest.mark.asyncio
async def test_hold_key_action_end_to_end():
    async with async_playwright() as p:
        ...
        # Press and auto-release (via duration)
...
@pytest.mark.asyncio
async def test_left_click_drag_action_end_to_end():
    async with async_playwright() as p:
        browser = await p.chromium.launch(headless=True)
        context = await browser.new_context()
        page = await context.new_page()
        ...
        # Drag again using explicit coordinates
        ...
        assert state2["moves"] >= state["moves"] + 1  # Verify drag motion occurred
