...
@pytest.mark.asyncio
async def test_apified_harvester_finds_trayectory_and_rebuilds_actions():
    ...
    with patch("aiohttp.ClientSession", return_value=session_mock):
        solution = await agent.find_trayectory(task)
    ...
    assert session_mock.post.call_args.args[0] == "http://localhost:9999/find_trayectory"
    _, kwargs = session_mock.post.call_args
    assert kwargs["json"]["url"] == "https://localhost"


@pytest.mark.asyncio
async def test_apified_harvester_accepts_trajectory_tool_calls():
    agent = ApifiedHarvester(base_url="http://localhost:9999", id="agent-1", name="StubHarvester")
    task = Task(url="https://example.com", prompt="Click CTA", web_project_id="dummy")
...
@pytest.mark.asyncio
async def test_apified_harvester_raises_when_endpoint_unavailable():
    agent = ApifiedHarvester(base_url="http://127.0.0.1:65500", id="agent-err", name="ErrAgent", timeout=1)
    task = Task(url="https://example.com", prompt="Click CTA", web_project_id="dummy")
...
async def test_apified_harvester_rejects_non_list_trajectory():
    ...
    response_mock.raise_for_status = MagicMock()
    response_mock.json = AsyncMock(return_value={"web_agent_id": "stub-agent", "trajectory": {"name": "click"}})
    post_mock = MagicMock()
    post_mock.__aenter__ = AsyncMock(return_value=response_mock)
    post_mock.__aexit__ = AsyncMock(return_value=None)
    session_mock = MagicMock()
...
def test_rewrite_to_remote_returns_none_for_none():
    assert ApifiedHarvester._rewrite_to_remote(None) is None
...
def test_rewrite_to_remote_relative_path():
    with patch("autoppia_iwa.src.web_agents.apified_one_shot_agent.DEMO_WEBS_ENDPOINT", "http://remote:8090"):
        result = ApifiedHarvester._rewrite_to_remote("/path")
...
def test_create_action_returns_none_for_invalid_raw():
    agent = ApifiedHarvester(base_url="http://localhost:5000")
